...
type Mode = 'LOGIN' | 'CREATE_CHARACTER' | 'CHOOSE_ROLE';
...
export default function LoginPage() {
    ...
    // Step 2: Submit final registration with chosen role
    const handleRegister = async (e: React.FormEvent) => {
        ...
        try {
            const res = await fetch('/api/auth/register', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ code: inviteCode.toUpperCase().trim(), playerRole: selectedRole, username: callsign, password: newPassword })
            });
            const data = await res.json();
            if (res.ok) { router.push('/'); router.refresh(); }
                ...
                {mode === 'LOGIN' && (
                    <>
                        ...
                        <form onSubmit={handleLogin} className="space-y-4">
                            <div>
                                <label className="block text-gray-400 text-xs font-bold mb-2 uppercase tracking-widest">Callsign</label>
                    ...
                    <>
                        ...
                        {selectedRole && (
                            <form onSubmit={handleRegister} className="space-y-4">
                                <div>
                                    <label className="block text-gray-400 text-xs font-bold mb-2 uppercase tracking-widest">Callsign / Handle</label>
                                    <input type="text" value={callsign} onChange={e => setCallsign(e.target.value)} required minLength={3}
                                ...
                                <button type="submit" disabled={loading}
                                    className={`${btnBase} ${loading ? 'bg-gray-700 text-gray-400 cursor-not-allowed' : 'bg-gradient-to-r from-purple-600 to-blue-600 hover:from-purple-500 hover:to-blue-500 text-white shadow-lg shadow-purple-900/30'}`}>
                                    {loading ? 'Creating Character...' : `Create ${ALL_ROLES.find(r => r.key === selectedRole)?.label ?? 'Character'} →`}
                                </button>
                            </form>
                        )}
